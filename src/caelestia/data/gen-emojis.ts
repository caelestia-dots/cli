...
async function fetchEmojis() {
  ...
  const link = `https://raw.githubusercontent.com/milesj/emojibase/refs/tags/emojibase-data%40${version}/packages/data/en/compact.raw.json`
...
async function fetchNF() {
  const version = "3.4.0"
  const link = `https://raw.githubusercontent.com/ryanoasis/nerd-fonts/refs/tags/v${version}/glyphnames.json`

  const res = await fetch(link)
  const data = await res.json()
  delete data["METADATA"] // Don't need this
